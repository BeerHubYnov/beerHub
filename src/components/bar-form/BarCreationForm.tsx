...
const BarCreationForm: React.FC = () => {
  const { user } = useAuth(); // Récupération de l'utilisateur authentifié
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!user?.id) { // Vérification si l'utilisateur est connecté
      setErrorMessage("Erreur : vous devez être connecté pour ajouter un bar.");
      return;
    }
    ...
    const barData = {
      ...
      id_User: user.id, // On récupère l'ID utilisateur dynamiquement
    };
  ...
  return (
    <div className="form-container">
      {!user ? ( 
        <p className="error">Vous devez être connecté pour ajouter un bar.</p>
      ) : (
        <>
